...
export default function Overview() {
  ...
  const stats = [
    { name: "Today's Orders", value: String(todaysOrders), change: `${pendingCount} pending`, icon: ClipboardList, color: '#3b82f6', bg: '#eff6ff', accent: '#3b82f6' },
    { name: 'Top Seller', value: topSeller ? topSeller[0] : '—', change: topSeller ? `${topSeller[1]} sold` : '—', icon: Flame, color: '#AD4928', bg: 'rgba(173,73,40,0.08)', accent: '#AD4928' },
    { name: "Today's Revenue", value: `Rs. ${todaysRevenue.toLocaleString()}`, change: `${orders.filter(o => o.status === 'Completed').length} done`, icon: DollarSign, color: '#10b981', bg: '#ecfdf5', accent: '#10b981' },
    { name: 'Active Tables', value: String(activeCustomers), change: 'Live', icon: Users, color: '#8b5cf6', bg: '#f5f3ff', accent: '#8b5cf6' },
  ];
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 32, paddingBottom: 40, fontFamily: "'Inter', system-ui, sans-serif" }}>

      {/* ── Header ── */}
      <div>
        <p className="font-bold uppercase" style={{ fontSize: 12, letterSpacing: '0.2em', color: '#AD4928', marginBottom: 8 }}>{today}</p>
        <h1 className="font-bold" style={{ fontSize: 32, color: '#3d2b1f', lineHeight: 1.2, fontFamily: "'Playfair Display', Georgia, serif" }}>
          Good Morning, Admin 👋
        </h1>
        <p style={{ fontSize: 16, color: '#9ca3af', fontStyle: 'italic', marginTop: 8 }}>
          Here's a live snapshot of your shop's performance today.
        </p>
      </div>

      {/* ── Stats ── */}
      <div id="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
        {stats.map((stat) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center justify-center" style={{ width: 44, height: 44, borderRadius: 14, background: stat.bg, color: stat.color }}>
                <stat.icon size={22} strokeWidth={2} />
              </div>
              <span className="flex items-center font-bold" style={{
                gap: 4, fontSize: 11, color: '#059669',
            ...
            <div>
              <p className="font-bold uppercase" style={{ fontSize: 10, color: '#9ca3af', letterSpacing: '0.15em', marginBottom: 6 }}>{stat.name}</p>
              <p className="font-bold" style={{ fontSize: 22, color: '#3d2b1f', lineHeight: 1.2 }}>{stat.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* ── Orders + Performance ── */}
      <div id="main-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24 }}>

        {/* Recent Orders */}
        <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #f3f4f6', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', overflow: 'hidden', minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          {/* Header */}
          <div className="flex items-center justify-between" style={{ padding: '24px 28px', borderBottom: '1px solid #fafafa', flexShrink: 0 }}>
            <div>
              <h2 className="font-bold" style={{ fontSize: 18, color: '#3d2b1f' }}>Recent Orders</h2>
              <p style={{ fontSize: 13, color: '#9ca3af', fontStyle: 'italic', marginTop: 4 }}>Live updates from table orders</p>
            </div>
            <button
              onClick={() => navigate('/admin/orders')}
              className="flex items-center font-bold uppercase transition-colors"
              style={{ gap: 4, fontSize: 12, letterSpacing: '0.06em', color: '#AD4928', cursor: 'pointer', background: 'none', border: 'none' }}
          ...
          <div style={{ flex: 1, overflowY: 'auto', maxHeight: 420 }}>
            {orders.length === 0 ? (
              <div style={{ padding: '48px 28px', textAlign: 'center' }}>
                ...
                <p className="font-bold" style={{ fontSize: 15, color: '#3d2b1f', marginBottom: 4 }}>No orders yet</p>
              ...
              orders.slice(0, 20).map((order, i) => {
                ...
                return (
                  <div key={order.id} className="flex items-center transition-colors"
                    style={{
                      padding: '18px 28px', gap: 16,
                      borderBottom: i < Math.min(orders.length, 20) - 1 ? '1px solid #fafafa' : 'none',
                      cursor: 'pointer',
                    }}
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="flex items-center" style={{ gap: 10, marginBottom: 5 }}>
                        <span className="font-bold" style={{ fontSize: 14, color: '#AD4928' }}>Table {order.table}</span>
                        <span className="flex items-center font-bold uppercase" style={{
                          gap: 5, fontSize: 10, letterSpacing: '0.06em',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
                      <div style={{ textAlign: 'right' }}>
                        <p className="font-bold" style={{ fontSize: 15, color: '#3d2b1f' }}>Rs. {order.total}</p>
                        <p className="flex items-center justify-end" style={{ fontSize: 11, color: '#9ca3af', gap: 4, marginTop: 3 }}>
        ...
        {/* Performance Card */}
        <div className="flex flex-col justify-between relative overflow-hidden" style={{
          ...
          {/* Decorative icon */}
          <div className="absolute pointer-events-none" style={{ bottom: -16, right: -16, opacity: 0.06 }}>
            <Coffee size={200} />
          </div>

          <div className="relative" style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
            <div>
              <p className="font-bold uppercase" style={{ fontSize: 10, letterSpacing: '0.25em', color: 'rgba(255,255,255,0.45)', marginBottom: 8 }}>Live Stats</p>
              <h3 className="font-bold" style={{ fontSize: 22, lineHeight: 1.3 }}>Shop Performance</h3>
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', fontStyle: 'italic', marginTop: 8, lineHeight: 1.6 }}>
                {todaysOrders > 0
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {[
                ...
                <div key={bar.label}>
                  ...
                  <div style={{ height: 8, width: '100%', background: 'rgba(255,255,255,0.1)', borderRadius: 999, overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: 999,
                      width: `${bar.pct}%`,
                      background: bar.pct > 50 ? '#ffffff' : 'rgba(255,255,255,0.55)',
                      boxShadow: bar.pct > 50 ? '0 0 10px rgba(255,255,255,0.3)' : 'none',
          ...
          <div className="relative" style={{ marginTop: 24, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
            <p className="font-bold uppercase" style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.2em', marginBottom: 6 }}>
              Today's Total Revenue
            </p>
            <div className="flex items-end justify-between">
              <p className="font-bold" style={{ fontSize: 32 }}>Rs. {todaysRevenue.toLocaleString()}</p>
